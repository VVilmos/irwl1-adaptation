...
Large neural networks have produced state-of-the-art results in numerous fields such as computer vision
 ...
 the model in a data center utilizing huge GPU clusters, liquid cooling, etc. At inference time, the clients usually access the model in the data center 
 also via the internet.

 However, with the proliferation of Internet-of-Things devices and applications, neural network models are required to run more efficiently on
 ...
 a small parameter count and an architecture which minimizes latency and floating-point operations. Depending on the deployment scenario, a model may
 ...
 the model refers to its ability to maintain its performance even on such inputs.  To sum up, a trained neural network has to satisfy accuracy, compactness and robustness requirements in order to become qualified for industrial
  ...
  In academia, there has been extensive research to improve these requirements, but most works tackled them separately \cite{effnet}, \cite{pruning_survey24}.
  ...
  distillation) and novel architectures have been proposed to achieve compactness while retaining accuracy. Finally, increasing robustness has been achieved by 
  ...
  Therefore, it is important to evaluate a technique tackling one of the three requirements on all three aspects. In our case, measuring the impact of model
...
and quantization, although it is also common to propose hybrid strategies \cite{survey}. Network pruning refers to evaluating weights or groups of weights
of the model based on a certain importance criteria and removing the ones which were considered the least important. This importance signal could be
the magnitude of weights, the sensitivity of loss with respect to them, or other heuristics. After pruning, a fine-tuning or retraining phase is applied using the 
...
which can lead to sub-optimal sparsity and to an increased complexity of tuning these thresholds for a deep network \cite{pruning_survey24}. In contrast, 
...
A basic assumption in traditional machine learning theory is that training and test samples follow the same distribution. However, in many applications this
...
Besides OOD inputs, an application-ready model is required to be robust to so-called adversarial inputs as well. By definition, adversarial inputs are 
...
vision models, various optimization strategies can be utilized to the find a specific perturbation applied to the input image to achieve its misclassification.   
