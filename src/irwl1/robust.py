...
def evaluate_deepfool_average_perturbation_norm(
	...
	"""Return the average L2 norm of the DeepFool perturbation per input sample."""

	try:
		import torchattacks
	except ImportError as exc:  # pragma: no cover - dependency guard
		raise ImportError(
			"torchattacks is required for DeepFool evaluation. Install it with `pip install torchattacks`."
		) from exc
...
# Shortened aliases for convenience
def deepfool_norm(
	model: torch.nn.Module,
	data_loader: torch.utils.data.DataLoader,
	device: torch.device | str | None = None,
	...
	"""Shortened alias for evaluate_deepfool_average_perturbation_norm (DataLoader version)."""
	return evaluate_deepfool_average_perturbation_norm(
		model=model,
...
def deepfool_norm_in_memory(
	...
	"""Return the average L2 norm of the DeepFool perturbation for in-memory tensors.

	Unlike deepfool_norm() which accepts a DataLoader, this function evaluates all
	images at once, which is efficient when data is already in memory.

	Args:
		model: Neural network to evaluate.
		images: Tensor of shape (N, C, H, W) with all images.
		labels: Tensor of shape (N,) with all labels.
		device: Device to run on (default: config.DEVICE).
		steps: DeepFool step count (default: 50).
		overshoot: DeepFool overshoot value (default: 0.02).
	Returns:
		Average L2 norm of minimal adversarial perturbations across all samples.
	"""
